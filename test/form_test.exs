defmodule AshPhoenix.FormTest do
  ...
  alias AshPhoenix.Test.{Domain, Artist, Author, Comment, OtherDomain, Post, PostWithDefault}
  ...
  describe "submit" do
    ...
    test "it raises an appropriate error when the incorrect domain is configured" do
      assert_raise Ash.Error.Invalid.ResourceNotAllowed,
                   ~r/Resource `AshPhoenix.Test.Post` is not accepted by AshPhoenix.Test.OtherDomain/,
                   fn ->
                     Post
                     |> Form.for_create(:create, domain: OtherDomain)
                     |> Form.validate(%{text: "text"})
                     |> Form.submit()
                   end
    end
